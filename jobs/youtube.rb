...
get '/chokapeek/youtube/callback' do
  $yt_access_token = params['access_token']
  $playlists = get_playlists()
  send_videos($playlists)
  redirect '/chokapeek'
end
...
def get_playlists()
  data = fetch_yt("#{SUBS}?part=id&channelId=#{ENV['YOUTUBE_CHAN_ID']}#{SUFFIX}")
  playlists = []
  loop do
    subs = []
    ...
    chans = fetch_yt("#{CHANS}?part=contentDetails&id=#{subs.join('%2C')}#{SUFFIX}")
...
def send_videos(playlists)
  ...
  videos.each_slice(50) do |chunk|
    true_videos.concat(fetch_yt("#{VIDS}?part=snippet&id=#{chunk.join('%2C')}#{SUFFIX}")['items'])
  end
